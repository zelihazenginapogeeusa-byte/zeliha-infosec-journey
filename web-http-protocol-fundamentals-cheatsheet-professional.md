...
SET usually ships with Kali/Parrot already, but here's how to grab and launch it if it's missing.
...
SET can also generate a malware payload and stand up the matching listener in one flow, useful for attachment or USB delivery scenarios.
...
Attacks that require the tester to be physically present rather than reaching the target over email or phone.
...
Guardrails that keep a social engineering engagement legal and defensible if anything is later questioned.
...
A single-page lookup for every command/menu path covered above.
