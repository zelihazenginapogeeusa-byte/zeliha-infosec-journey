...
5. Check the affected host/account for any post-click anomalous activity (new mailbox rules, unusual sign-ins, unexpected process execution) — cross-reference [`splunk-siem-investigation-playbook.md`](../01-SOC-and-SIEM-Analysis/splunk-siem-investigation-playbook.md) Playbooks 3–4 if sign-in or PowerShell anomalies are found.
...
3. If it ran: pull the process tree spawned from the document/executable (e.g., `WINWORD.EXE` → `powershell.exe` is a classic macro-dropper pattern) — cross-reference [`splunk-siem-investigation-playbook.md`](../01-SOC-and-SIEM-Analysis/splunk-siem-investigation-playbook.md) Playbook 4 for the PowerShell-specific triage steps.
