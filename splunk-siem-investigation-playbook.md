...
4. Cross-reference the decoded command against [`command-line-obfuscation-evasion-cheatsheet.md`](command-line-obfuscation-evasion-cheatsheet.md) patterns.
...
3. Check for persistence artifacts created around the same time (run keys, scheduled tasks, services) — see [`memory-and-disk-forensics-quickref.md`](memory-and-disk-forensics-quickref.md).
4. Extract any available hash/IOC and check reputation (see [`reputation-lookup-tools-cheatsheet-professional.md`](reputation-lookup-tools-cheatsheet-professional.md)).
